        ...
        <ul class="contract" >
          <li>
            <div class="item-shenling-li">
              <label>抵押合同:</label>
              <input id="putDiya" v-model="messages1" type="number" :placeholder="holder1" @focus="cleared1()" @blur="filler1()">
            </div>
          </li>
          <li>
            <div class="item-shenling-li">
              <label>信用合同:</label>
              <input  id="putXinyong" v-model="messages2" type="number" :placeholder="holder2" @focus="cleared2()" @blur="filler2()">
            </div>
          </li>
          <li>
            <div class="item-shenling-li">
              <label>质押合同:</label>
              <input id="putZhiya" v-model="messages3" type="number" :placeholder="holder3" @focus="cleared3()" @blur="filler3()">
            </div>
          </li>
          <li>
            <div class="item-shenling-li">
              <label>申请原因:</label>
              <textarea @click="moveKey($event)" v-model="textmessage" style="border: solid 1px #cccccc;resize:none;" id="putReason" name="" rows="5"  cols="" placeholder="简述你的原由......	"></textarea>
            </div>
          </li>
...
export default {
  ...
  methods:{
    ...
    tijiao(){
        ...
        axios.get(host+"/app/applycontract?userid="+this.userid+"&dcount="+this.messages1+"&zcount="+this.messages3+"&wcount="+this.messages2+"&reason="+this.textmessage+"&uuid="+this.uuid,qs.stringify({
        })).then(response=>{
