<template>
  <div id="appdiyadai" style="text-align: left;overflow-Y:scroll">
    <ContractTop tle="抵押贷" tleurl="/index" v-bind:submit="headone" v-on:submitone="submitone" style="position:fixed;top:0px;z-index:999;background:#fff;" ></ContractTop>
    <mt-navbar v-model="selected" style="width:100%;position:fixed;top:44px;z-index:999;background:#fff;" value=1>
      <div style="width:620px;">
        ...
        <mt-tab-item id="6" v-if="yijian">审批意见</mt-tab-item>
      </div>
    </mt-navbar>
    ...
    <mt-tab-container v-model="selected"  :swipeable='true' style="margin-top:98px;">
      ...
      <mt-tab-container-item id="2">
        <liuchengfujian v-on:liuchengflag="liuchengflag" v-bind:showsave="showsave"  v-bind:liuchengall="liuchengcontent" v-bind:contracttype="1" v-bind:contractid="contractid" v-bind:contractnum="contractnum" v-bind:showor="showor"></liuchengfujian>
      </mt-tab-container-item>
      <mt-tab-container-item id="3">
        <jiaojiedan v-on:jiaojiedanflag="jiaojiedanflag" v-bind:jiaojiedanall="jiaojiedancontent" v-bind:showsave="showsave" v-bind:showor="showor" v-bind:contractid="contractid" v-bind:contractnum="contractnum" v-bind:dingjin="dingjin"></jiaojiedan>
      </mt-tab-container-item>
...
<script>
  ...
  import diyadaicontract from '@/components/old/diyadaicontract';
    ...
    created(){
      ...
      axios.post(host+"/app/loadGuarantyContractMain?contractnum="+this.contractnum,{
        ...
        .then(response=>{
          ...
          this.dingjin=response.data.data.diya.depositAmount;
          if(this.dianwei>1.8){
            this.typeid=0;
          }else{
             this.typeid=1;
      ...
      approve:function(data){
       Indicator.open();
        axios.get(host+"/app/approve?userid=" +this.userid + "&nodeType=0&typeid=" + this.typeid + "&step=" +this.step + "&processinstanceid=" +this.processinstanceid + "&num=0" + "&contractid=" +this.contractid + "&auditoropinion=" + data+ "&uuid=" +this.uuid,qs.stringify({
           ...
           this.newarr.approveIdea=data;
            this.newarr.contractnum=this.contractnum;
          if(response.data.code=="200"){
            if(this.step==1){
