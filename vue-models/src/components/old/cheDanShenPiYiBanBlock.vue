  ...
  <div id="appBlock" style="overflow-Y:scroll;">
    <div id="refreshContainer" class="mui-content mui-scroll-wrapper" >
        <ul class="contract" v-for="item in items">
          ...
          <li class="uptime4">
            <p class="Guardian_2">
              <span class="Guardian_2w">审批意见：</span><span class="Guardian_2zhuang">{{item.opinion}}</span>
            </p>
          </li>
          <li class="uptime4">
            <p class="uptime_1">
              <span class="uptime_1t">审批时间：</span><span class="uptime_1tc">{{item.date}}</span>
            </p>
          </li>
          <li class="uptime4">
            <p class="uptime_1">
...
export default {
  ...
  methods:{
    load() {
      ...
      axios.get(host+"/app/abandondone?userid="+this.userId,qs.stringify({
      })).then(response=>{
