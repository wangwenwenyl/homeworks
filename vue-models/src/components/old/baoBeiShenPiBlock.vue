  ...
  <div id="appBlock" style="overflow-Y:scroll;">
    <div id="refreshContainer" class="mui-content mui-scroll-wrapper" >
        <ul class="contract" v-for="item in items">
          ...
          <li class="uptime4">
            <p class="uptime_1">
              <span class="uptime_1t">合同创建日期：</span><span class="uptime_1tc">{{item.updatetime}}</span>
            </p>
          </li>
          <li class="btn5">
            <div class="zuofei" @click="path(item)">
...
export default {
  ...
  methods:{
      ...
      axios.post(host+"/app/getrecordcontractlist?uname="+this.uname+"&type=2&userid="+this.userid,qs.stringify({
      })).then(response=>{
      ...
      path(item){
        this.contractnum = item.processname.match(/^编号(.*?)质押合同报备流程$/)[1];
        console.log(this.contractnum);
        this.$router.push({ name: 'oldbaobei', params:{step:item.step,nodeType:0,processinstanceid:item.processinstanceid,contractnum:this.contractnum,typeid:1,contractid:item.id}});
      },
      handleTopChange() {
          // 加载更多数据
          setTimeout(() => {
